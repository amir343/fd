...
%% Using exponential probability distribution
%% ie. putting higher weights to recent heartbeats
...
-record(state,
        {nodes          = []         :: [term()],
         node_histories = dict:new() :: dict:dict(),
...
handle_call({is_alive, Node}, _From,
            State=#state{node_histories = Hs, config = C}) ->
  {reply, is_available(Node, Hs, C), State};

handle_call({get_histories, Node}, _From, State=#state{node_histories = Hs}) ->
  Result =
    case dict:find(Node, Hs) of
      {ok, #node_history{node = _, latest_timestamp = _, history = H}} ->
        H;
      _ ->
        not_found
    end,
...
handle_cast({alive, Node},
            State=#state{nodes = Ns, node_histories = Hs, config = C}) ->
...
send_heartbeat_to_nodes(Nodes, HeartbeatInt) ->
  ...
  timer:apply_after(HeartbeatInt, gen_server, cast,
                    [{fd, node()}, send_heartbeat]).

%%
%% @private
-spec deregister_node/3 :: (node(), [node()],dict:dict())
                           -> {[node()], dict:dict()}.
...
%%
%% @private
-spec received_alive/3 :: (node(), dict:dict(), config()) -> dict:dict().
received_alive(Node, Histories, C) ->
  Now = timestamp_millis(),

  case dict:find(Node, Histories) of
    {ok, #node_history{node = Node,
                       latest_timestamp = LatestTs,
                       history = History}} ->
      case LatestTs =:= 0 of
        true ->
          NHistory = heartbeat_history:append(History,
                                              C#config.max_sample_size div 2,
                                              C#config.max_sample_size),
    ...
    error ->
      dict:store(Node,
                 #node_history{node = Node,
                               latest_timestamp = 0,
                               history = heartbeat_history:new()},
                 Histories)
  end.

%%
%% @private
-spec is_available/3 :: (node(), dict:dict(), config())
                        -> { node_available | node_unavailable, float()}.
...
%%
%% @private
-spec phi/3 :: (node(), dict:dict(), pos_integer()) -> float() | infinity.
